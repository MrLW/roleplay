...
public class EnhancedByteBuf {
	 ...
	 * // 1、先写集合的长度 this.writeInt(objs.size()); // 2、写list中具体内容 try { for (int i =
	 * 0; i < objs.size(); i++) { Object obj = objs.element(); // 复杂数据类型 if
	 * (!BeanUtil.isBaseDataType(obj.getClass())) { writeObj(obj); } else { //
	 * 基本数据类型 writeBaseObj(obj); }
	 * 
